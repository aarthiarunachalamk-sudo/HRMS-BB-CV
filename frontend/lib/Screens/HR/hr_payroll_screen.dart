...
class _HrPayrollScreenState extends State<HrPayrollScreen> {
  ...
  List<Map<String, dynamic>> _calculatedPayslips = const [];
  ...
  List<Map<String, dynamic>> get _employees =>
      hrList(widget.data, 'total_employees_list');
  List<Map<String, dynamic>> get _payslips => _calculatedPayslips.isNotEmpty
      ? _calculatedPayslips
      : hrList(widget.data, 'payroll_items');
  int get _total => int.tryParse('${widget.data['total_employees'] ?? 0}') ?? 0;
  int get _processed =>
      int.tryParse('${widget.data['payroll_processed'] ?? 0}') ?? 0;
  int get _pending =>
      int.tryParse('${widget.data['payroll_pending'] ?? 0}') ?? 0;

  List<_PayrollIssue> get _issues {
    final issues = <_PayrollIssue>[];
    final validation = _processValidation.isNotEmpty
        ? _processValidation
        : hrList(widget.data, 'payroll_validation');
      ...
      if (employee['bank_missing'] == true) {
        issues.add(
          _PayrollIssue(
            ...
            detail: 'Account number or IFSC is unavailable',
            colorKey: 'critical',
          ),
        );
      }
      if (employee['attendance_conflict'] == true) {
        issues.add(
          _PayrollIssue(
            ...
            detail:
                '${employee['attendance_days'] ?? 0}/${employee['expected_attendance_days'] ?? 0} working days recorded',
            colorKey: 'warning',
          ),
        );
      }
      if (employee['salary_configured'] != true) {
        issues.add(
          _PayrollIssue(
            ...
            detail: 'Configure salary components before payroll',
            colorKey: 'critical',
          ),
        );
      }
      if (employee['performance_submitted'] != true) {
        issues.add(
          _PayrollIssue(
            id: '$id-performance',
            employeeId: id,
            name: name,
            title: 'Performance review missing',
            detail: 'Submit the monthly or quarterly review before calculation',
            colorKey: 'warning',
          ),
        );
      }
      ...
      children: [
          ...
          children: [
            Expanded(
              child: _metric(
                context,
                'Gross Pay',
                _money(widget.data['payroll_gross']),
                c.primary,
        ...
        if (_issues.isEmpty)
          ...
          _attentionCard(
            ...
            '${_issues.where((e) => e.colorKey == 'warning').length} attendance conflicts',
            'Review payable days before calculation',
            c.warning,
          ),
          const SizedBox(height: 8),
          _attentionCard(
  ...
  Widget _validationHub(BuildContext context) {
    ...
    final unresolved = issues
        .where((issue) => !_resolvedIssues.contains(issue.id))
        .toList();
    return ListView(
      ...
      children: [
          ...
          children: [
            ...
            Expanded(
              child: _filterTile(
                context,
                'Critical',
                '${issues.where((e) => e.colorKey == 'critical').length}',
                c.danger,
                false,
              ),
            ),
            ...
            Expanded(
              child: _filterTile(
                context,
                'Warnings',
                '${issues.where((e) => e.colorKey == 'warning').length}',
                c.warning,
                false,
              ),
            ),
        ...
        if (issues.isEmpty)
          _successCard(
            context,
            'All employee records are ready for calculation.',
          )
        else
          ...issues.map(
            (issue) => Padding(
              padding: const EdgeInsets.only(bottom: 9),
              child: _issueCard(context, issue),
            ),
          ),
        const SizedBox(height: 8),
        HrCard(
          child: Row(
            children: [
              CircleAvatar(
                ...
                child: Icon(Icons.check_rounded, color: c.primary),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${issues.length - unresolved.length} of ${issues.length} resolved',
                      style: TextStyle(
                        color: c.text,
                        fontSize: 12,
                        fontWeight: FontWeight.w900,
                    ...
                    LinearProgressIndicator(
                      value: issues.isEmpty
                          ? 1
                          : (issues.length - unresolved.length) / issues.length,
                      color: c.primary,
                      backgroundColor: c.border,
                    ),
                  ],
  ...
  Widget _salaryWorkspace(BuildContext context) {
    final c = HrPalette.of(context);
    final employee = _employees.isEmpty
        ? <String, dynamic>{}
        : _employees[_employeeIndex.clamp(0, _employees.length - 1).toInt()];
    ...
    final gross =
        double.tryParse(
          '${slip?['total_earnings'] ?? slip?['gross_salary'] ?? 0}',
        ) ??
        0.0;
    final deductions =
        double.tryParse('${slip?['total_deductions'] ?? 0}') ?? 0.0;
    final performanceScore =
        double.tryParse('${slip?['performance_score'] ?? 0}') ?? 0.0;
    final performanceIncentive =
        double.tryParse('${slip?['performance_incentive'] ?? 0}') ?? 0.0;
      ...
      children: [
        ...
        HrCard(
          child: Row(
            children: [
              IconButton(
                onPressed: _employeeIndex > 0
                    ? () => setState(() => _employeeIndex--)
                    : null,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '$employeeId  •  ${employee['role'] ?? employee['designation'] ?? ''}',
                      style: TextStyle(
                        color: c.primary,
                        fontSize: 9,
              ...
              IconButton(
                onPressed: _employeeIndex + 1 < _employees.length
                    ? () => setState(() => _employeeIndex++)
                    : null,
        ...
        HrCard(
          child: Row(
            children: [
              Expanded(
                child: _salaryArc(context, 'Earnings', gross, c.primary),
              ),
              ...
              Container(width: 1, height: 70, color: c.border),
              Expanded(
                child: _salaryArc(context, 'Deductions', deductions, c.danger),
              ),
            ],
          ),
        ),
        ...
        HrCard(
          child: Row(
            children: [
              Icon(Icons.trending_up_rounded, color: c.success, size: 30),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Text(
                      'Score ${performanceScore.toStringAsFixed(2)} / 5.00 • Prorated by payable attendance',
        ...
        HrCard(
          child: Row(
            children: [
              Icon(Icons.verified_user_outlined, color: c.success, size: 34),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Text(
                      'Salary matches available attendance and policy data.',
                      style: TextStyle(color: c.muted, fontSize: 9),
                    ),
                  ],
                ),
          ...
          children: [
              ...
              child: OutlinedButton.icon(
                onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Salary adjustment saving is not connected yet.',
                      ),
                    ),
                  );
                },
            ...
            Expanded(
              child: ElevatedButton.icon(
                onPressed: () => _advance('calculate', 3),
                style: ElevatedButton.styleFrom(
                  backgroundColor: c.primary,
                  foregroundColor: Colors.white,
                ),
    ...
    return ListView(
      ...
      children: [
        _flowHeader(
          context,
          'Publish Payroll',
          _published ? 'Payroll completed' : 'Final review and publishing',
          () => setState(() => _stage = 2),
        ),
        const SizedBox(height: 12),
        HrCard(
          child: Column(
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: _publishMetric(context, '$_total', 'Employees'),
                  ),
                  Container(width: 1, height: 40, color: c.border),
                  Expanded(
                    child: _publishMetric(
                      context,
                      _money(widget.data['payroll_net']),
                      'Total Payout',
                    ),
          ...
          child: Column(
            ...
            children: [
              ...
              _switchRow(
                ...
                'Generate Payslips',
                _generatePayslips,
                (v) => setState(() => _generatePayslips = v),
              ),
              _switchRow(
                context,
        ...
        if (!_published)
          ElevatedButton.icon(
            ...
            label: Text(
              _publishing ? 'Publishing Payroll...' : 'Publish Payroll',
            ),
          )
        else
          HrCard(
            child: Column(
              children: [
                CircleAvatar(
                  radius: 31,
                  backgroundColor: c.success.withAlpha(30),
                  child: Icon(Icons.check_rounded, color: c.success, size: 38),
                ),
                const SizedBox(height: 10),
                Text(
                  'Payroll Published',
                ...
                const SizedBox(height: 4),
                Text(
                  '$_total payslips generated successfully',
                  style: TextStyle(color: c.muted, fontSize: 10),
                ),
                      ...
                      child: OutlinedButton(
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Payroll report download is not connected yet.',
                              ),
                            ),
                          );
                        },
  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '${result['message'] ?? 'Payroll published successfully'}',
          ),
        ),
      );
    } catch (error) {
  ...
  void _applyProcess(Map<String, dynamic> result) {
    ...
    if (process is Map) {
      ...
      _resolvedIssues
        ..clear()
        ..addAll(
          (process['resolved_issues'] is List
                  ? process['resolved_issues'] as List
                  : const [])
              .map((item) => '$item'),
        );
  ...
  Widget _pageTitle(BuildContext context, String title, {String? badge}) {
    final c = HrPalette.of(context);
    return Row(
      children: [
        Expanded(
        ...
        Expanded(
          child: Column(
            children: [
              ...
              Text(subtitle, style: TextStyle(color: c.muted, fontSize: 9)),
            ],
          ),
        ),
        Icon(Icons.more_vert_rounded, color: c.muted),
      ],
    );
  }
  ...
  Widget _sectionTitle(BuildContext context, String title, String action) {
    ...
    return Row(
      children: [
        ...
        Text(action, style: TextStyle(color: c.muted, fontSize: 9)),
      ],
    );
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(detail, style: TextStyle(color: c.muted, fontSize: 9)),
              ],
            ),
          ),
          OutlinedButton(
  ...
  Widget _filterTile(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: selected ? color.withAlpha(28) : c.surface,
  ...
  Widget _issueCard(BuildContext context, _PayrollIssue issue) {
    final c = HrPalette.of(context);
    final color = issue.colorKey == 'critical' ? c.danger : c.warning;
    final resolved = _resolvedIssues.contains(issue.id);
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(9),
        border: Border(
          left: BorderSide(color: resolved ? c.success : color, width: 4),
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Text(
              issue.name.isEmpty ? '?' : issue.name[0],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  resolved ? 'Resolved' : issue.title,
                  style: TextStyle(
                    color: resolved ? c.success : color,
                    fontSize: 10,
                    fontWeight: FontWeight.w900,
                  ),
                ),
          ...
          OutlinedButton(
            onPressed: resolved ? null : () => _resolveIssue(issue.id),
            style: OutlinedButton.styleFrom(
              foregroundColor: color,
              side: BorderSide(color: color),
              visualDensity: VisualDensity.compact,
            ),
            child: Text(resolved ? 'Done' : 'Resolve'),
          ),
        ],
      ),
    );
  ...
  Widget _successCard(BuildContext context, String text) {
    final c = HrPalette.of(context);
    return HrCard(
      child: Row(
        children: [
          Icon(Icons.verified_rounded, color: c.success, size: 30),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              text,
  ...
  Widget _salaryArc(
    ...
    return Column(
      children: [
        Icon(Icons.donut_large_rounded, color: color, size: 31),
        const SizedBox(height: 5),
        Text(label, style: TextStyle(color: c.muted, fontSize: 9)),
        Text(
          _money(amount),
  ...
  Widget _publishMetric(BuildContext context, String value, String label) {
    final c = HrPalette.of(context);
    return Column(
      children: [
        Text(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 11,
            backgroundColor: done ? c.success : c.row,
            child: Icon(
              done ? Icons.check_rounded : Icons.schedule_rounded,
              color: done ? Colors.white : c.primary,
              size: 13,
            ),
          ),
  ...
  Widget _switchRow(
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          Icon(icon, color: c.muted, size: 17),
          const SizedBox(width: 8),
          Expanded(
            child: Text(title, style: TextStyle(color: c.text, fontSize: 10)),
          ),
          Switch(value: value, onChanged: changed, activeColor: c.primary),
        ],
      ),
    );
  }
  ...
  List<(String, double)> _componentRows(Object? value) {
    ...
    return value.entries
        .map(
          (entry) =>
              (entry.key.toString(), double.tryParse('${entry.value}') ?? 0.0),
        )
        .toList();
  }
  ...
  String _monthPeriod() {
    ...
    return '01 ${_monthShort(now.month)} – $last ${_monthShort(now.month)} ${now.year}';
  }
