...
class _CeoLeaveIntelligenceScreenState
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  subtitle: Text('${_isoDate(range.start)} to ${_isoDate(range.end)}'),
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${result['message'] ?? 'Company leave announced'}')),
          );
          setState(_load);
        }
      } catch (error) {
  ...
  Widget build(BuildContext context) => CeoShell(
    ...
    trailing: PopupMenuButton<int>(
      ...
      itemBuilder: (_) => [
        ...
        const PopupMenuItem(
          value: 99,
          child: Row(children: [Icon(Icons.refresh), SizedBox(width: 8), Text('Refresh data')]),
        ),
      ],
    ),
    child: FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          children: [
            _FlowTabs(selected: _page, onSelected: (i) => setState(() => _page = i)),
  ...
  Widget _dashboard(Map<String, dynamic> data) {
    ...
    return ListView(
      ...
      children: [
        CeoMetricGrid(cards: [
          CeoMetric('Total Requests', '${summary['total'] ?? 0}', '', Icons.event_note, CeoColors.cyan, onTap: () => setState(() { _status = 'all'; _page = 1; })),
          CeoMetric('Pending', '${summary['pending'] ?? 0}', '', Icons.pending_actions, CeoColors.gold, onTap: () => setState(() => _page = 3)),
          CeoMetric('Approved', '${summary['approved'] ?? 0}', '', Icons.event_available, CeoColors.green, onTap: () => setState(() { _status = 'approved'; _page = 1; })),
          CeoMetric('Rejected', '${summary['rejected'] ?? 0}', '', Icons.event_busy, Colors.redAccent, onTap: () => setState(() { _status = 'rejected'; _page = 1; })),
        ]),
        ...
        const SizedBox(height: 14),
        _heading('Recent Leave Requests', action: 'View all', onTap: () => setState(() => _page = 1)),
  ...
  Widget _requests(Map<String, dynamic> data, {bool pendingOnly = false}) {
    final requests = _maps(data['requests']).where((item) {
      final status = '${item['overall_status'] ?? item['status']}'.toLowerCase();
      final effectiveStatus = pendingOnly ? 'pending' : _status;
      final matchesStatus = effectiveStatus == 'all' || status.contains(effectiveStatus);
      final haystack = '${item['name']} ${item['employee_id']} ${item['leave_type']}'.toLowerCase();
    ...
    return Column(children: [
      Padding(
        ...
        child: TextField(
          ...
          decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Search requests'),
        ),
      ),
      if (!pendingOnly)
        SingleChildScrollView(
          ...
          child: Row(children: ['all', 'pending', 'approved', 'rejected'].map((status) => Padding(
      ...
      Expanded(
        child: requests.isEmpty
            ...
            : ListView(padding: const EdgeInsets.symmetric(horizontal: 14), children: requests.map(_requestCard).toList()),
      ),
    ]);
  }
  ...
  Widget _calendar(Map<String, dynamic> data) {
    final calendarItems = [
      ..._maps(data['calendar']),
      ..._maps(data['company_leaves']).map((item) => {
        ...item,
        'company_leave': true,
      }),
    ];
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      Row(children: [
        ...
        Expanded(child: Center(child: title('${_months[_month.month - 1]} ${_month.year}', 15))),
        IconButton(onPressed: () => _changeMonth(1), icon: const Icon(Icons.chevron_right)),
      ]),
      CeoCard(child: CalendarDatePicker(
      ...
      _heading('Leave on ${_selectedDate.day} ${_months[_selectedDate.month - 1]}'),
      if (selected.isEmpty) CeoCard(child: Center(child: muted('No leave on selected date', 11))),
      ...selected.map((item) {
      ...
      ...balances.map((item) => CeoCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Expanded(child: title('${item['type']}', 12)), muted('${item['available']} / ${item['entitlement']} days', 10)]),
        const SizedBox(height: 8),
  ...
  Widget _analytics(Map<String, dynamic> data) {
    ...
    final maxTrend = trend.fold<double>(1, (max, item) {
      final total = _number(item['approved']) + _number(item['pending']) + _number(item['rejected']);
      return total > max ? total : max;
    });
    return ListView(padding: const EdgeInsets.all(14), children: [
      CeoMetricGrid(cards: [
        CeoMetric('Total Leaves', '${summary['total'] ?? 0}', '', Icons.event_note, CeoColors.cyan),
        CeoMetric('Approved', '${summary['approved'] ?? 0}', '', Icons.check_circle, CeoColors.green),
        CeoMetric('Pending', '${summary['pending'] ?? 0}', '', Icons.schedule, CeoColors.gold),
        CeoMetric('Rejected', '${summary['rejected'] ?? 0}', '', Icons.cancel, Colors.redAccent),
      ]),
      const SizedBox(height: 14),
      _heading('Monthly Leave Trend'),
      CeoCard(child: Column(children: trend.map((item) {
        final total = _number(item['approved']) + _number(item['pending']) + _number(item['rejected']);
        return Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Row(children: [
          ...
          Expanded(child: LinearProgressIndicator(value: total / maxTrend, color: CeoColors.green, backgroundColor: CeoColors.border)),
          SizedBox(width: 30, child: Text('${total.toInt()}', textAlign: TextAlign.end)),
        ]));
      }).toList())),
      const SizedBox(height: 8),
      _heading('Leave by Type'),
      ...types.map((item) => CeoListTile(
  ...
  Widget _requestCard(Map<String, dynamic> item) => CeoListTile(
    ...
    subtitle: '${item['leave_type'] ?? 'Leave'} • ${item['from_date']} • ${item['status']}',
    color: _statusColor('${item['overall_status'] ?? item['status']}'),
    onTap: () => _openRequest(item),
  );

  Widget _heading(String text, {String? action, VoidCallback? onTap}) => Padding(
    padding: const EdgeInsets.only(bottom: 9),
    child: Row(children: [Expanded(child: title(text, 14)), if (action != null) TextButton(onPressed: onTap, child: Text(action))]),
  );

  Widget _navButton(String label, IconData icon, int page) => Padding(
    padding: const EdgeInsets.only(bottom: 8),
    child: OutlinedButton.icon(onPressed: () => setState(() => _page = page), icon: Icon(icon), label: Text(label)),
  );
  ...
  static const _titles = ['Leave Intelligence', 'Leave Requests', 'Calendar & Balance', 'Approve / Reject', 'Leave Analytics'];
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: ListView.separated(
      ...
      itemBuilder: (_, index) => ChoiceChip(
        label: Text('${index + 1}. ${_CeoLeaveIntelligenceScreenState._titles[index]}'),
