...
class CeoPayrollOverviewScreen extends StatefulWidget {
  ...
  @override
  State<CeoPayrollOverviewScreen> createState() =>
      _CeoPayrollOverviewScreenState();
}
...
class _CeoPayrollOverviewScreenState extends State<CeoPayrollOverviewScreen> {
  ...
  Future<void> _action(String action, {bool declaration = false}) async {
    ...
    if (destructive) {
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          title: Text(
            action == 'publish' ? 'Publish Payroll' : 'Calculate Payroll',
          ),
          content: Text(
            action == 'publish'
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          '${result['message'] ?? (result['success'] == true ? 'Payroll updated.' : 'Unable to update payroll.')}',
        ),
  ...
  Widget? _headerAction() {
    if (_page == 0) {
      return IconButton(
        ...
        icon: const Icon(Icons.calendar_month, color: CeoColors.cyan),
      );
    }
    if (_page == 1) {
      return IconButton(
        onPressed: () => _message(
          'Processing stages use live employee validation and payroll records.',
        ),
        icon: const Icon(Icons.help_outline, color: CeoColors.cyan),
      );
    }
    if (_page == 2) {
      return IconButton(
        onPressed: () => setState(() => _showDeductions = !_showDeductions),
        icon: const Icon(Icons.filter_alt_outlined, color: CeoColors.cyan),
      );
    }
    if (_page == 3) {
      return IconButton(
        onPressed: _selectedPayslip == null
            ? null
            : () => _openPayslip(_selectedPayslip!),
        icon: const Icon(Icons.file_download_outlined, color: CeoColors.cyan),
      );
    }
    return null;
  }
  ...
  Future<void> _openPayslip(Map<String, dynamic> payslip) async {
    ...
    try {
      ...
      if (opened != true && mounted)
        _message('Unable to open the payslip PDF.');
      ...
      children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [muted('Pay Period', 9), title('${data['period']}', 14)],
          ),
        ),
  ...
  Widget _dashboard(Map<String, dynamic> data) {
    ...
    final maxDepartment = departments.fold<double>(
      1,
      (max, item) => _num(item['amount']) > max ? _num(item['amount']) : max,
    );
    return ListView(
                      ...
                      child: Stack(
                        ...
                        children: [
                          CircularProgressIndicator(
                            value: _num(employees['active']) == 0
                                ? 0
                                : _num(employees['processed']) /
                                      _num(employees['active']),
                            strokeWidth: 8,
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            muted(_money(item['amount']), 10),
                          ],
                        ),
                        const SizedBox(height: 5),
                        LinearProgressIndicator(
                          value: _num(item['amount']) / maxDepartment,
                          color: CeoColors.cyan,
      ...
      children: [
              ...
              ...steps.asMap().entries.map(
                (entry) => ListTile(
                  ...
                  leading: CircleAvatar(
                    backgroundColor: entry.key <= stage
                        ? CeoColors.green
                        : CeoColors.cardAlt,
                    child: entry.key < stage
        ...
        CeoMetricGrid(
          cards: [
            ...
            CeoMetric(
              'Ready',
              '${validation.where((item) => item['ready'] == true).length}',
              '',
              Icons.verified,
              CeoColors.green,
            ),
            CeoMetric(
              ...
              '',
              Icons.warning_amber,
              unresolved == 0 ? CeoColors.green : Colors.redAccent,
            ),
            CeoMetric(
              'Processed',
              '${_map(data['employees'])['processed'] ?? 0}',
              '',
              Icons.task_alt,
              CeoColors.purple,
            ),
        ...
        if (stage == 0)
          FilledButton(
            ...
            child: const Text('Start Salary Processing'),
          ),
        if (stage == 1)
          FilledButton(
            onPressed: unresolved == 0 ? () => _action('validate') : null,
            child: Text(
              unresolved == 0
                  ? 'Complete Validation'
                  : 'Resolve $unresolved Issues First',
            ),
          ),
        if (stage == 2)
          FilledButton(
        ...
        if (stage >= 3)
          FilledButton(
            onPressed: () => _goReview(data),
            child: const Text('Review Payslips'),
          ),
        const SizedBox(height: 8),
        OutlinedButton(
          onPressed: () => _showValidation(validation),
          child: const Text('View Processing Details'),
        ),
      ],
    );
  }
    ...
    return ListView(
      ...
      children: [
        SegmentedButton<bool>(
          segments: const [
            ButtonSegment(value: false, label: Text('Earnings')),
            ButtonSegment(value: true, label: Text('Deductions')),
          ],
          selected: {_showDeductions},
          onSelectionChanged: (value) =>
              setState(() => _showDeductions = value.first),
        ),
        const SizedBox(height: 12),
        CeoMetricGrid(
          cards: [
        ...
        const SizedBox(height: 8),
        ...earnings.map(
          (item) => _breakdownRow(item, totals['earnings'], false),
        ),
        const SizedBox(height: 6),
        title('Deductions Breakdown', 14),
        const SizedBox(height: 8),
        ...deductions.map(
          (item) => _breakdownRow(item, totals['deductions'], true),
        ),
        FilledButton(
          onPressed: () => _goReview(data),
          child: const Text('Review Employee Payslips'),
        ),
      ],
    );
  }
  ...
  Widget _review(Map<String, dynamic> data) {
    final payslips = _maps(data['payslips'])
        .where(
          (item) => '${item['employee_name']} ${item['employee_id']}'
              .toLowerCase()
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'Search employee payslip',
            ),
          ),
        ),
        if (selected != null)
            ...
            child: ListView(
              ...
              children: [
                DropdownButtonFormField<Map<String, dynamic>>(
                  value: payslips.contains(selected)
                      ? selected
                      : payslips.first,
                  isExpanded: true,
                  ...
                  items: payslips
                      ...
                      .toList(),
                  onChanged: (value) =>
                      setState(() => _selectedPayslip = value),
                ),
                const SizedBox(height: 12),
                SegmentedButton<bool>(
                  segments: const [
                  ...
                  onSelectionChanged: (value) =>
                      setState(() => _showPaymentHistory = value.first),
                ),
                const SizedBox(height: 10),
                if (_showPaymentHistory)
                  _paymentHistoryCard(selected)
                else
                  _payslipCard(selected),
                FilledButton(
                  onPressed: () => setState(() => _page = 4),
                  child: const Text('Next: Approve & Publish'),
                ),
              ],
            ),
          )
          ...
          Expanded(
            child: Center(
              child: muted('No calculated payslips for this period', 12),
            ),
          ),
      ],
    );
  ...
  Widget _payslipCard(Map<String, dynamic> payslip) => CeoCard(
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        title('Earnings', 12),
        ..._map(payslip['earnings']).entries.map(
          (entry) => _amountRow(entry.key, entry.value, CeoColors.green),
        ),
        _amountRow('Total Earnings', payslip['total_earnings'], CeoColors.cyan),
        const Divider(color: CeoColors.border),
        title('Deductions', 12),
        ..._map(payslip['deductions']).entries.map(
          (entry) => _amountRow(entry.key, entry.value, Colors.redAccent),
        ),
        _amountRow(
          'Total Deductions',
    ...
    return ListView(
      ...
      children: [
        ...
        CeoCard(
          child: Column(
            ...
            children: [
              ...
              _workflowRow(
                'Validated',
                '${process['validated_at'] ?? '-'}',
                _hasValue(process['validated_at']),
              ),
              _workflowRow(
                'Calculated',
                '${process['calculated_at'] ?? '-'}',
                _hasValue(process['calculated_at']),
              ),
              _workflowRow(
                'Approved & Published',
                '${process['published_at'] ?? 'Pending'}',
                published,
              ),
            ],
          ),
        ...
        CheckboxListTile(
          ...
          onChanged: published
              ? null
              : (value) => setState(() => _declaration = value ?? false),
          title: const Text(
            'I confirm that this payroll has been reviewed and verified.',
          ),
        ...
        CheckboxListTile(
          ...
          onChanged: published
              ? null
              : (value) => setState(() => _authorization = value ?? false),
          title: const Text(
            'I authorize publication and payment initiation for this payroll period.',
          ),
          controlAffinity: ListTileControlAffinity.leading,
        ),
        FilledButton.icon(
          onPressed: published || !_declaration || !_authorization
              ? null
              : () => _action('publish', declaration: true),
          icon: Icon(published ? Icons.check_circle : Icons.send),
          label: Text(
            published ? 'Payroll Published' : 'Approve & Publish Payroll',
          ),
        ),
      ],
    );
  ...
  void _showValidation(List<Map<String, dynamic>> items) =>
      showModalBottomSheet<void>(
        ...
        builder: (context) => DraggableScrollableSheet(
          ...
          builder: (_, controller) => ListView(
            ...
            children: [
              title('Payroll Validation', 16),
              const SizedBox(height: 12),
              ...items.map(
                (item) => ListTile(
                  leading: Icon(
                    item['ready'] == true ? Icons.check_circle : Icons.warning,
                    color: item['ready'] == true
                        ? CeoColors.green
                        : Colors.redAccent,
                  ),
                  title: Text(
                  ...
                  subtitle: Text(
                    item['ready'] == true
                        ? 'Ready for payroll'
                        : [
                            if (item['bank_missing'] == true)
                              'Bank details missing',
                            if (item['attendance_conflict'] == true)
                              'Attendance incomplete',
                            if (item['salary_configured'] != true)
      ...
      child: Row(
        children: [
          Icon(
            ...
            color: deduction ? Colors.redAccent : CeoColors.cyan,
          ),
          const SizedBox(width: 10),
          Expanded(child: title('${item['name']}', 11)),
          Column(
            ...
            children: [
              ...
              muted(
                '${total == 0 ? '0.0' : (_num(item['amount']) / total * 100).toStringAsFixed(1)}%',
                9,
              ),
            ],
          ),
  ...
  Widget _workflowRow(String label, String detail, bool complete) => ListTile(
    contentPadding: EdgeInsets.zero,
    leading: CircleAvatar(
      backgroundColor: complete ? CeoColors.green : CeoColors.cardAlt,
      child: Icon(complete ? Icons.check : Icons.schedule, color: Colors.white),
    ),
